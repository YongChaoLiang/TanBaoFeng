...
def preprocess(h, info, compositions):
    ...
    res = res.reshape((len(compositions), 5, 5))
...
if __name__ == '__main__':
    ...
    compositions = np.asarray(data['composition_1'])
    res = preprocess(h, info, compositions)
    res = np.expand_dims(res, axis=1)
    np.save('data/data.npy', res)

    gfa_label = np.asarray(data['GFA'])
    np.save('data/label.npy',gfa_label)
