...
if __name__ == '__main__':
    ...
    t_data = data[data['GFA'] == 'BMG'][['composition_1', 'Tg', 'Tx', 'Tl']]
    # print(list(t_data))

    idx = np.asarray(~t_data.isnull().T.any())
    idx = np.argwhere(idx == True).flatten()
    # print(t_data)

    compositions = np.asarray(t_data['composition_1'])[idx]
    print(len(compositions))
    print(compositions[359])
    print(compositions[360])
    # res = preprocess(h, info, compositions)
    # np.save('data/scale_data.npy', res)
    #
    # value = np.asarray(t_data[['Tg', 'Tx', 'Tl']])[idx]
    # # value = np.log(value)
